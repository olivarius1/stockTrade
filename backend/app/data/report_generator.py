...
# 填充可选因子的默认值（用户通过命令行提供的优先）
factor_values = {}
for fk in OPTIONAL_FACTOR_KEYS:
    if fk in optional_factors:
...
# 2. DB模式：从SQLite数据库读取（增量更新后使用全量数据）
...
<header class="report-header">
  ...
  <div class="meta">2026年7月 &middot; 基于2025年报及2026Q1最新数据</div>
</header>
...
<section id="s3">
  ...
  <div class="chart-figure">
    ...
    <div id="chart-backtest" style="width:100%;height:500px;"></div>
    <p>图表说明：蓝色折线为综合估值分（0-100），橙色折线为收盘价（元），紫色折线为PE(TTM)，灰色柱状为总市值。绿色虚线为70分低估分界线，红色虚线为40分高估分界线。估值分越高代表越被低估。</p>
  </div>
</section>
<section id="s4">
...
(function() {{
  ...
  chart.setOption({{
    ...
    legend: {{ data: ['估值分(0-100)', '收盘价(元)', 'PE(TTM)', '总市值(亿元)'], bottom: 10, textStyle: {{ color: '#1a1a1a', fontSize: 13 }} }},
    grid: {{ left: 70, right: 85, top: 60, bottom: 100 }},
    xAxis: {{
      type: 'category', data: dates,
      axisLabel: {{ color: '#6b6b6b', fontSize: 11, rotate: 45, formatter: function(v) {{ return v.substring(5); }} }},
    ...
    series: [
      ...
      {{ name: '收盘价(元)', type: 'line', data: closes, yAxisIndex: 1, lineStyle: {{ color: '#c75b2a', width: 1.2 }}, itemStyle: {{ color: '#c75b2a' }}, symbol: 'none', z: 3 }},
      {{ name: 'PE(TTM)', type: 'line', data: peTTMs, yAxisIndex: 1, lineStyle: {{ color: '#b8860b88', width: 1.5 }}, itemStyle: {{ color: '#b8860b' }}, symbol: 'none', z: 2 }},
      {{ name: '总市值(亿元)', type: 'bar', data: marketCaps, yAxisIndex: 1, itemStyle: {{ color: '#f0eeeb', opacity: 0.5 }}, barWidth: '60%', barGap: '-100%', z: 1 }}
    ]
  }});
...
function openFullscreenChart() {{
  ...
  _fsChart.setOption({{
    ...
    legend: {{ data: ['估值分(0-100)', '收盘价(元)', 'PE(TTM)', '总市值(亿元)'], bottom: 10, textStyle: {{ color: '#9ca3af', fontSize: 12 }} }},
    grid: {{ left: 65, right: 80, top: 50, bottom: 90 }},
    xAxis: {{
      type: 'category', data: dates,
      axisLabel: {{ color: '#6b7280', fontSize: 10, rotate: 45, formatter: function(v) {{ return v.substring(5); }} }},
    ...
    series: [
      ...
      {{ name: '收盘价(元)', type: 'line', data: closes, yAxisIndex: 1, lineStyle: {{ color: '#fbbf24', width: 1.0 }}, itemStyle: {{ color: '#fbbf24' }}, symbol: 'none', z: 3 }},
      {{ name: 'PE(TTM)', type: 'line', data: peTTMs, yAxisIndex: 1, lineStyle: {{ color: '#a78bfa88', width: 1.2 }}, itemStyle: {{ color: '#a78bfa' }}, symbol: 'none', z: 2 }},
      {{ name: '总市值(亿元)', type: 'bar', data: marketCaps, yAxisIndex: 1, itemStyle: {{ color: '#374151', opacity: 0.6 }}, barWidth: '60%', barGap: '-100%', z: 1 }}
    ]
  }});
  var resizeTimer;
